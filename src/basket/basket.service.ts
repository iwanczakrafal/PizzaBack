...
import {
    ...
    ClearBasketRes,
    GetBasketStatsRes,
    GetTotalBasketPriceRes, ProductsFromBasketRes,
...
export class BasketService {
    ...
    async getAllBasketProductsForUser(user: User): Promise<ProductsFromBasketRes> {

        const productsToMap = await this.dataSource
            .createQueryBuilder()
        ...
        const products = productsToMap.map((product) => (
            product.option ?
                {
                    id: product.id,
                    count: product.count,
                    productItem: {
                        ...
                        isSpecial:product.productItem.isSpecial,
                    },
                    option: {
